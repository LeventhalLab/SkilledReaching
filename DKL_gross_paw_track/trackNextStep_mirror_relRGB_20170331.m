function [fullMask] = trackNextStep_mirror_relRGB_20170331( image_ud, BGimg_ud, fundMat, greenBGmask, prevMask, boxRegions, pawPref,varargin)
...
%             pawBehindFrontPanel = true;
            ...
            if any(frontPanelMask_ROI(:)) && (max(grDist_adj_int(:)) < int_grDistThresh_res(2)) %&& any(tempMask_ext(:))
                grayMask_int = (gray_img > int_grayRange_nearFrontPanel(ii,1)) & (gray_img < int_grayRange_nearFrontPanel(ii,2)) & intMask;
                grayMask{ii} = grayMask_ext | grayMask_int;
                abs_grdiffMask_int = (abs_grdiff > min_int_abs_grDiff) & intMask;
                int_tempMask_res = (grDist_adj_ROI > int_grDistThresh_res(1)) & grMask & gbMask & BGdiff_mask_ROI{ii} & abs_grdiffMask_int & grayMask_int; %int_frontPanel_neighbors;
                int_tempMask_lib = (grDist_adj_ROI > int_grDistThresh_lib(1)) & grMask & gbMask & BGdiff_mask_ROI{ii} & abs_grdiffMask_int & grayMask_int; % & int_frontPanel_neighbors & grayMask_int;
                int_tempMask = imreconstruct(int_tempMask_res, int_tempMask_lib);
            else
                grayMask_int = (gray_img > int_grayRange(ii,1)) & (gray_img < int_grayRange(ii,2)) & intMask;
                grayMask{ii} = grayMask_ext | grayMask_int;
%                 abs_grdiffMask_int = (abs_grdiff > min_abs_grDiff(ii)) & intMask;
                abs_grdiffMask_int = (abs_grdiff > min_int_abs_grDiff) & intMask;
                int_tempMask_res = (grDist_adj_ROI > int_grDistThresh_res(2)) & grMask & gbMask & BGdiff_mask_ROI{ii} & abs_grdiffMask_int & grayMask_int; %int_frontPanel_neighbors;
                int_tempMask_lib = (grDist_adj_ROI > int_grDistThresh_lib(2)) & grMask & gbMask & BGdiff_mask_ROI{ii} & abs_grdiffMask_int & grayMask_int; % & int_frontPanel_neighbors & grayMask_int;
                int_tempMask = imreconstruct(int_tempMask_res, int_tempMask_lib);
            end
        else
...
%         end
        ...
        belowShelfExcludeMask = true(size(grayMask{ii}));
    else        
        if pawBehindFrontPanel
            grayMask{ii} = (gray_img > int_grayRange(ii,1)) & (gray_img < int_grayRange(ii,2));
        else
            grayMask{ii} = (gray_img > grayRange(ii,1)) & (gray_img < grayRange(ii,2));
        end
        ...
        if any(belowShelf_ROI(:)) && any(shelfMask_ROI(:))
            ...
            belowShelfExcludeMask = belowShelf_tempMask | ~belowShelf_ROI;
        else
            belowShelf_tempMask = false(size(grayMask{ii}));
        end
        
%         if any(belowShelf_tempMask(:))    % check that there is white fur near any paw detected below the shelf (for R30, 5/1, there's a smudge of green near the elbow that needs to be rejected)
%             pawNeighbors = imdilate(belowShelf_tempMask,strel('disk',5)) & belowShelf_ROI & ~belowShelf_tempMask;
%             meanGray = gray_img(pawNeighbors);
%             % calculate % of neighbors with gray values > neighborThresh
%             numNeighborPix = sum(pawNeighbors(:));
%             numBrightNeighbors = sum(meanGray > neighborThresh);
%             pctBrightPix = numBrightNeighbors / numNeighborPix;
% %             if median(meanGray) < neighborThresh
%             if pctBrightPix < neighborBrightThresh
%                 belowShelf_tempMask = false(size(grayMask{ii}));
%                 belowShelfExcludeMask = belowShelfExcludeMask & ~imdilate(belowShelf_ROI,strel('disk',3));
%             end
%         end
    end
    ...
    tempMask = (imreconstruct(tempMask_res,tempMask_lib) | int_tempMask | belowShelf_tempMask) & belowShelfExcludeMask;
