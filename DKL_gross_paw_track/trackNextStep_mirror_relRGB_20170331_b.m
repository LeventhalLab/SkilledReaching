...
grayRange_ext = [0.2,0.8            % could eventually use this for when the paw is outside the front of the box...
                 0.2,0.6];
int_grayRange = [0.16,0.45
                 0.1,0.45];
int_grayRange_nearFrontPanel = [0.16,0.45
                                0.15,0.45];
...
%         dilated_bbox(ii,1) = max(dilated_bbox(ii,1) - 50,1);
%         dilated_bbox(ii,2) = max(dilated_bbox(ii,2) - 50,1);
%         dilated_bbox(ii,3) = dilated_bbox(ii,3) + 50;
%     end
    if ii == 2   
...
%     grDist_ROI = sqrt(gr_diff_clipped.^2 + gb_diff_clipped.^2);
    ...
    if ii == 2
        ...
        if any(intMask(:))
            ...
            grayMask{ii} = (gray_img > grayRange(ii,1)) & (gray_img < grayRange(ii,2)) & intMask;
            grayMask{ii} = grayMask{ii} | grayMask_ext;
        end
%         
...
%         end
        ...
        if pawBehindFrontPanel
            int_grayMask = (gray_img > int_grayRange(ii,1)) & (gray_img < int_grayRange(ii,2));
        else
            int_grayMask = false(size(grayMask{ii}));
        end
        grayMask{ii} = grayMask{ii} | int_grayMask;
