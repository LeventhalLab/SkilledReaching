...
in_adjust_ext = [0,0,0;ones(1,3)*min(0.8,(gray_lim_ext + 0.1))];
out_adjust_ext = [ones(1,3)*max(0.2,(gray_lim_ext - 0.1));1,1,1];

gray_lim_idx = find(cumCount < totCount*stretch_hist_limit_int,1,'last');
gray_lim_int = g_bins(gray_lim_idx);
in_adjust_int = [0,0,0;ones(1,3)*min(0.8,(gray_lim_int + 0.1))];
out_adjust_int = [ones(1,3)*max(0.2,(gray_lim_int - 0.1));1,1,1];
