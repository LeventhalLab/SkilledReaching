function initPawMask = find_initPawMask_greenPaw_relRGB_20170208_d( image_ud, BGimg_ud, sr_ratInfo, session_mp, boxCalibration, boxRegions, greenBGmask, varargin )
...
rel_r = rel_im(:,:,1); rel_g = rel_im(:,:,2); rel_b = rel_im(:,:,3);
rel_gr_diff = rel_g - rel_r;
rel_gb_diff = rel_g - rel_b;
im_grDist = sqrt(rel_gr_diff.^2 + rel_gb_diff.^2);
lh = stretchlim(im_grDist,[0.002,0.998]);
im_grDist_adj = imadjust(im_grDist,lh);
...
%     grDist_adj = imadjust(grDist);
    grDist_adj = im_grDist_adj(ROI(iView,2):ROI(iView,2)+ROI(iView,4),...
                               ROI(iView,1):ROI(iView,1)+ROI(iView,3));
...
%         end
    ...
    grMask{iView} = view_rel_grdiff{iView} > min_gr_diff(iView);
    gbMask{iView} = view_rel_gbdiff{iView} > min_gb_diff(iView);
