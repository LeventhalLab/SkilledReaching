function h=plot1dDistanceScores(allAlignedXyzDistPawCenters,plotFrames,superTitle)
    ...
    for i=1:numel(allAlignedXyzDistPawCenters)
        ...
        velfilt = diff(smoothn(alignedXyzDistPawCenters(startFrame:plotFrames),10,'robust'));
        hold on;
                ...
                switch(scoreData(i,2))
                    ...
                    case {2,3,4,7}
                        ...
                        if(isempty(plot2Avg))
                            ...
                            plot2Avg = (velfilt+plot2Avg)/2;
                        end
                end
                colormapline(startFrame:(plotFrames-1),velfilt,[]);
            end
        end
    end
    ...
    for k=1:2
        ...
        %view(h(k),azel); % az,el
        xlabel(h(k),'frames');
        ylabel(h(k),'velocity (mm/frame)');
        %zlabel(h(k),'z');
        %legend on;
        grid(h(k));
        box(h(k));
        axis(h(k),[0 plotFrames -5 3]); % x y z
        
        switch k
            case 1
                title(h(k),'First Trial Success - 1');
                plot(startFrame:(plotFrames-1),plot1Avg,'Color','magenta','Marker','o');
            case 2
                title(h(k),'Unsuccessful - {2,3,4,7}');
                plot(startFrame:(plotFrames-1),plot2Avg,'Color','magenta','Marker','o');
        end
    end
end