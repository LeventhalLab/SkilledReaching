function plot_max_v_acrossExperiments(exptSummary)
%
% exptSummary - types
%   1 - chr2 during
%   2 - chr2 between
...
saveName = 'max_v_across_expt.pdf';
...
for i_exptType = 1 : length(exptTypeIdx)
    ...
    toPlot = squeeze(nanmean(curSummary.mean_pd_v,2));
    numValidPts = sum(~isnan(curSummary.mean_pd_v),2);
    e_bars = nanstd(curSummary.mean_pd_v,0,2) ./ sqrt(numValidPts);

    set(gca,'ylim',max_v_lim,...
        'xtick',[1,2,3,12,13,22],...
        'xticklabel',[1,2,1,10,1,10],...
        'ytick',[max_v_lim(1), mean(max_v_lim), max_v_lim(2)],...
        'fontsize',16,...
        'fontname','arial');
...
%     h_leg.Location = 'southeast';
    ...
    if i_exptType == 1
        ylabel('max velocity (mm/s)','fontname','arial','fontsize',18)
        set(gca,'yticklabel',[max_v_lim(1), mean(max_v_lim), max_v_lim(2)]);
    else
        set(gca,'yticklabel',[]);
    end
