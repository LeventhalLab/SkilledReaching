...
for i_rat = 4:4%4%length(sr_ratInfo)
...
%         sessionDate = triDataFiles(iSession).name(7:14);
        ...
        for iVid = 1 : length(vidList)
            ...
            pawData = load_pawTrackData_relRGB(processedDir, trialNum);
...
%                     ud_pt = undistortPoints(squeeze(pawData(iFrame,:,iView)),cameraParams);
                    if isempty(pawData.points2d{iView,iFrame});continue;end
                    ud_pt = pawData.points2d{iView,iFrame};
