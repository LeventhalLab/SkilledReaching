...
%     sharedX_ratRootFolder = fullfile(sharedX_DLCoutput_path,ratID);
    ...
    for iSession = startSession : 1: endSession
        ...
        test_name = sprintf('%s_box*_dlc_out.mat', ratID);
        mat_from_python_list = dir(test_name);
        C = textscan(mat_from_python_list.name,[ratID '_box%02d']);
        
        [direct_bp, mirror_bp, direct_p, mirror_p, direct_pts_ud, mirror_pts_ud, pawPref] = ...
            load_mat_from_python(mat_from_python_list.name);
