...
% script to refine the initial 3D reconstructions of reaching trajectories
% 1. clean up data (e.g., ignore unreliable points)
% 2. calculate paw orientation based on digit locations
% 3. figure out when the paw first breached the slot (needed later to
%    automatically identify reaches)
% 4. figure out where the pellet was located to use as the origin for
%    (x,y,z) coordinate system
% 5. recalculate bodypart coordinates with respect to the pellet-as-origin
...
% •	Subfolders RXXXX_YYYYMMDDz_direct/left/right that contain the actual DLC output files and metadata from cropping (i.e., cropping coordinates, frame rate, etc)
...
ratIDs_with_new_date_format = [284];    % dates for this rat were entered in a different format. hard-coded work-around
...
% parent directory for DLC output, cropping metadata, kinematics analyses
...
for i_rat = 1:numRatFolders   % change loop limits if only want to analyze specific rats
    ...
    for iSession = startSession : 1 : endSession
        ...
        for iTrial = 1 : numTrials
            ...
            % figure out which points identified by DLC are unreliable
        ...
        % repeat analyses for trials in which the pellet wasn't found,
        % using the average location of the pellet on trials in which it
        % was found as the origin
        for iTrial = 1 : numTrials
