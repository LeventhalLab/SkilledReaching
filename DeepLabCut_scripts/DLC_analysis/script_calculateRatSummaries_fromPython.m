...
%     histo_ratFolder = fullfile(histoFolder,ratID);
    ...
    sessionDates = sessionDatesFromDirectories(sessionDirectories);
    
    sessionCSV = [ratID '_sessions.csv'];
    sessionCSV = fullfile(ratRootFolder, sessionCSV);
    sessionTable = readSessionInfoTable(sessionCSV);
    
%     sessions_analyzed = getRetrainingThroughOcclusionSessions(sessionTable);
    analysis_dates = sessionDatesFromDirectories(sessionDirectories);
    sessions_analyzed = matchSessionsToDates(sessionTable, analysis_dates);
    ...
    ratSummary = initializeRatSummaryStruct_6OHDA(ratID,validTrialOutcomes,validOutcomeNames,sessions_analyzed,thisRatInfo,z_interp_digits);
    ...
    for iSession = startSession : endSession
        ...
        ratSummary.interp_aperture_traj{iSession} = temp_aperture_traj;
        ratSummary.interp_orientation_traj{iSession} = temp_orientation_traj;
        
    end
    
%     ratSummary.aperture_traj = temp_aperture_traj;
    cd(ratRootFolder);
