function [ PCA_paw_hist, PCA_nonpaw_hist, PCAbinEdges ] = pawPCAhists2( local_relRGB, localPawMask, PCAcoeff, numHistBins )
%UNTITLED4 Summary of this function goes here
%   Detailed explanation goes here

% PCA_paw_hist = zeros(numHistBins,3);
% PCA_nonpaw_hist = zeros(numHistBins,3);
PCAbinEdges = zeros(numHistBins+1,2);    % number of bins x 2 views x 3 principal components
...
transformed_rgb = colorArray * PCAcoeff(:,1:2);    % only look at the first two principal components


pca_paw = zeros(sum(paw_indices),2);
PCA_nonPaw = zeros(sum(~paw_indices),2);
for ii = 1 : 2
...
[PCA_paw_hist,~,~] = histcounts2(pca_paw(:,1),pca_paw(:,2),PCAbinEdges(:,1),PCAbinEdges(:,2),'normalization','probability');
[PCA_nonpaw_hist,~,~] = histcounts2(PCA_nonPaw(:,1),PCA_nonPaw(:,2),PCAbinEdges(:,1),PCAbinEdges(:,2),'normalization','probability');
