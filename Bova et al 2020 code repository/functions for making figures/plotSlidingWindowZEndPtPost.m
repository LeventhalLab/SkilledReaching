function plotSlidingWindowZEndPtPost(indivSlidingWindow,indivSlidingWindowPost,sess,numBins,sessType)
...
if sessType == 'o'
    ...
    for i_rat = 1:numRats   % collect data from last day of occlusion
        occData(:,i_rat) = indivSlidingWindow(2).digEnd(:,22,i_rat)*-1;
    end
...
elseif sessType == 'l'
    ...
    for i_sess = 1:2    % collect data from first 2 days of laser on during
        for i_rat = 1:numRats
            lasData(:,i_rat,i_sess) = indivSlidingWindowPost.digEnd(:,i_sess,i_rat)*-1;
        end 
    end 
