function withinSessStatsOrientation(indivSlidingWindow,groups,session,numBins)

for i_bin = 1:numBins
        curData = NaN(size(indivSlidingWindow(i_grp).aperture,3),2);
    for i_grp = groups
        
        data = indivSlidingWindow(i_grp).orienation(:,:,:);
        data(data == 0) = NaN;
        ...
        for i_rat = 1:size(indivSlidingWindow(i_grp).orienation,3)
