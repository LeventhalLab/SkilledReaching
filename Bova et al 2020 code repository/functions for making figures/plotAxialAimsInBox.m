function plotAxialAimsInBox(aimsSummary)

for i_grp = 1:4 % calculate average individual rats
    avgAxialScores(i_grp,:) = nanmean(aimsSummary(i_grp).axial);
end

grpAvgs = nanmean(avgAxialScores,2);    % calculate group averages

numDataPts = sum(~isnan(avgAxialScores),2); % std. dev.
errBars = nanstd(avgAxialScores,0,2)./sqrt(numDataPts);
...
for i_grp = 1:4 % plot individual averages
    xes = ones(1,6)*i_grp;
    scatter(xes,avgAxialScores(i_grp,:),indMarkerSize,'MarkerEdgeColor',indivColor,...
