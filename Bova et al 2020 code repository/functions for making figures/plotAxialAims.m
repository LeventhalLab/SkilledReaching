function plotAxialAims(cylData,groups,combineChR)
...
avgAxial = nanmean(indivAxial2,2);  % calculate average
erBars = nanstd(indivAxial2,0,2)./sqrt(size(indivAxial2,2));    % calculate s.e.m.

% put data in format for plotting (rows laser power, columns groups ChR2
% day 1, ChR2 day 2, EYFP day 1, EYFP day 2)
avgPlot = [avgAxial(:,:,1)'; avgAxial(:,:,2)'; avgAxial(:,:,3)'; avgAxial(:,:,4)'; avgAxial(:,:,5)'; avgAxial(:,:,6)'];
...
for i_pow = 1:6
    ...
    for i_rat = 1:size(indivAxial2,2)
        plot(xVals,indivAxial2(1:2,i_rat,i_pow),'Color',indivColor,'LineWidth',.5)
    end
end 
