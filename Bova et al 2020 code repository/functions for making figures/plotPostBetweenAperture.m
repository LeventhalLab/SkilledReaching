function plotPostBetweenAperture(exptSummaryHisto,btwPostSummary)

occlAvgEnd = nanmean(exptSummaryHisto(2).mean_end_aperture(22,:),2);    % average last occlusion session
errBars(1,1) = nanstd(exptSummaryHisto(2).mean_end_aperture(22,:),0,2)./sqrt(sum(~isnan(exptSummaryHisto(2).mean_end_aperture(22,:))));
avgEnd = nanmean(btwPostSummary.mean_end_aperture(1:2,:),2);    % average laser on sessions
errBars(1,2:3) = nanstd(btwPostSummary.mean_end_aperture(1:2,:),0,2)./sqrt(sum(~isnan(btwPostSummary.mean_end_aperture(1:2,:)),2));

indivData = [exptSummaryHisto(2).mean_end_aperture(22,:); btwPostSummary.mean_end_aperture(1:2,:)];
