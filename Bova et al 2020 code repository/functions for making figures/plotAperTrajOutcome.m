function h_fig = plotAperTrajOutcome(exptOutcomeSummary,i_grp,exclRat)
...
for i_out = 1:8
    for i_pt = 1:size(exptOutcomeSummary(i_grp).mean_aperture_traj,2)
        numDataPts = sum(~isnan(exptOutcomeSummary(i_grp).mean_aperture_traj(:,i_pt,i_out,ratsInPlot)),4);
        avgData(:,i_pt,i_out) = nanmean(exptOutcomeSummary(i_grp).mean_aperture_traj(:,i_pt,i_out,ratsInPlot),4);
        for i_sess = 1:22
