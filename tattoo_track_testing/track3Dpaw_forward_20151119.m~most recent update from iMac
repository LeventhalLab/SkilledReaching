function digitTrajectories = track3Dpaw_forward_20151119(video, ...
                                         BGimg_ud, ...
                                         refImageTime, ...
                                         initDigitMasks, ...
                                         init_mask_bbox, ...
...
pelletTrack = struct(...
    ...
    'bbox', bbox, ...
    'color', 'multi', ...
    'visiblemask1', pelletMasks{1}, ...
    'visiblemask2', pelletMasks{2}, ...
    'visiblemask3', pelletMasks{3}, ...
    'fullmask1',
    'fullmask2',
    'fullmask3',
    'prev_vismask1', pelletMasks{1}, ...
    'prev_vismask2', pelletMasks{2}, ...
    'prev_vismask3', pelletMasks{3}, ...
    'prev_fullmask1', {}, ...
    'prev_fullmask2', {}, ...
    'prev_fullmask3', {}, ...
...
%       each digit, 4th dimension is the view (1 = direct, 2 = mirror)
    ...
    newTrack = struct(...
        ...
        'bbox', bbox, ...
        'color', curColor, ...
        'visiblemask1', squeeze(digitMasks{1}(:,:,ii)), ...
        'visiblemask2', squeeze(digitMasks{2}(:,:,ii)), ...
        'visiblemask3', squeeze(digitMasks{3}(:,:,ii)), ...
        'fullmask1',
        'fullmask2',
        'fullmask3',
        'prev_vismask1', squeeze(digitMasks{1}(:,:,ii)), ...
        'prev_vismask2', squeeze(digitMasks{2}(:,:,ii)), ...
        'prev_vismask3', squeeze(digitMasks{3}(:,:,ii)), ...
        'prev_fullmask1', {}, ...
        'prev_fullmask2', {}, ...
        'prev_fullmask3', {}, ...
...
%                                 tracks(sameColIdx(iDigit)).visiblemask1;
%             fullDigitMasks{2} = fullDigitMasks{2} | ...
%                                 tracks(sameColIdx(iDigit)).visiblemask2;
        end
    end    % end for iColor...
...
%       view (1 = direct, 2 = mirror)
    ...
    fullCenterMask(mask_bbox(1,2) : mask_bbox(1,2) + mask_bbox(1,4),...
                   mask_bbox(1,1) : mask_bbox(1,1) + mask_bbox(1,3)) = tracks(num_elements_to_track-1).visiblemask1;
    ...
    tracks(num_elements_to_track-1).visiblemask3 = ...
        fullMirrorMask(mask_bbox(3,2) : mask_bbox(3,2) + mask_bbox(3,4),...
    ...
    fullMirrorMask(mask_bbox(2,2) : mask_bbox(2,2) + mask_bbox(2,4),...
                   mask_bbox(2,1) : mask_bbox(2,1) + mask_bbox(2,3)) = tracks(num_elements_to_track-1).visiblemask2;
	projMask = pawProjectionMask(fullCenterMask, fundMat(:,:,1), [h,w]);
    fullMirrorMask = fullMirrorMask & projMask;
    tracks(num_elements_to_track-1).visiblemask2 = ...
        fullMirrorMask(mask_bbox(2,2) : mask_bbox(2,2) + mask_bbox(2,4),...
...
function newTrack = checkSingleTrack(prevTrack, ...
    ...
    newTrack.prev_vismask1 = newTrack.visiblemask1;
    newTrack.prev_vismask2 = newTrack.visiblemask2;
    newTrack.visiblemask1 = prelimMask{1,1};
    newTrack.visiblemask2 = prelimMask{1,2};
...
%             digLabelMask{iTrack,iView} = bwlabel(prelimMask{iTrack,iView});
        ...
        elseif sum(numBlobs(iTrack,:)) == 1    % only one of 4 possible blobs is visible
            ...
            newTracks(iTrack).visiblemask1 = false(mask_bbox(1,4:-1:3) + 1);
            newTracks(iTrack).visiblemask2 = false(mask_bbox(2,4:-1:3) + 1);
...
newTracks(iTrack).visiblemask1 = centerMask;
newTracks(iTrack).visiblemask2 = mirrorMask;
...
newTracks(iTrack).prev_vismask1 = newTracks(iTrack).visiblemask1;
newTracks(iTrack).prev_vismask2 = newTracks(iTrack).visiblemask2;
...
if epiDist < trackCheck.maxEpiLineDist
    ...
    if iTrack == find(distFromTracks == min(distFromTracks))
        ...
        newTracks(iTrack).visiblemask1 = prelimMask{iTrack,1};
        newTracks(iTrack).visiblemask2 = prelimMask{iTrack,2};
        ...
        newTracks(iTrack).visiblemask1 = false(size(prelimMask{iTrack,1}));
        newTracks(iTrack).visiblemask2 = false(size(prelimMask{iTrack,2}));
...
%         dorsum_mask = (labMat > 0);
    ...
    if iView == 2
        tracks(1).prev_vismask2  = tracks(1).visiblemask2;
        frontOverlap = prevMask{iView} & frontMask & angledRegion{iView};
        tracks(1).visiblemask2 = dorsumRegionMask{iView} | frontOverlap;
        tracks(1).visiblemask2 = multiRegionConvexHullMask(tracks(1).visiblemask2);
        tracks(1).visiblemask2 = tracks(1).visiblemask2 & candidateRegion{iView};

        % calculate projection into center view
        if any(dorsumRegionMask{iView}(:))
            projMask = calcProjMask(tracks(1).visiblemask2, ...
                                    trackingBoxParams.F(:,:,1)', ...
                                    mask_bbox(iView,:), ...
                                    imSize);
            projMask = projMask(mask_bbox(1,2) : mask_bbox(1,2) + mask_bbox(1,4), ...
...
currentMask = cell(1,2);
digitMasks = cell(1,2);
digitMasks{1} = false(size(tracks(2).visiblemask1));
digitMasks{2} = false(size(tracks(2).visiblemask2));
firstMask = cell(1,2);
lastMask = cell(1,2);
for ii = 2 : length(tracks)-2
    currentMask{1} = tracks(ii).visiblemask1;
    currentMask{2} = tracks(ii).visiblemask2;
