function maskedPaw = identifyMirrorDigits_dorsum_20150708(image, pawMask, rat_metadata, varargin)
%
% usage
%
...
decorrStretchMean  = [127.5 100.0 127.5     % to isolate dorsum of paw
...
masked_hsv_enh = zeros(numObjects, size(image,1),size(image,2),size(image,3));
...
dMask = zeros(size(image,1),size(image,2),numObjects);
SE = strel('disk',2);
s = regionprops(pawMask,'centroid');                      
wholePawCentroid = s.Centroid;
fullDigitMask = false(size(image,1),size(image,2));
digitCentroids = zeros(numObjects-1,2);
for ii = 2 : numObjects    % CONSIDER PUTTING A CHECK HERE THAT IF ALL DIGITS AREN'T FOUND, TRY ANOTHER IMAGE
    ...
    tempMask = HSVthreshold(squeeze(masked_hsv_enh(ii,:,:,:)), ...
                            [hueLimits(colorIdx,:), minSaturation, 1.0, 0.0, 1.0]);
...
for ii = 2 : numObjects
    fullDigitMask = fullDigitMask | dMask(:,:,ii);
end
...
[~,P] = imseggeodesic(image, idx_erode, middle_erode, ring_erode);
[~, P2] = imseggeodesic(image, pd_erode, pinky_erode, middle_erode);
