...
if LocalDataFolderStatus > 0;
    save(LocalPawPointFilename,'RatData','-v6');
else
    mkdir(fullfile(LocalSaveFolder,'Paw_Point_Marking_Data',RatID,SessionName));
    save(LocalPawPointFilename,'RatData','-v6');
end
save(PawPointFilename,'RatData','-v6');
...
for iVideo = iVideo:length(RatData(SessNum).VideoFiles);
    ...
    if RatData(SessNum).VideoFiles(iVideo).Score% || RatData(SessNum).VideoFiles(iVideo).Score == 7
        ...
        temp = GUIcreateManualPoints_2015_06_19(RatData,SessNum,iVideo,StartFrame,'interval',8,'end_frame',StartFrame+80);
