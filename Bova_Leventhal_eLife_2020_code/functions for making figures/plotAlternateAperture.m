function h_fig = plotAlternateAperture(alternateKinematicSummaryHisto,plotArch,plotOffOn)

alternateAperture = putAlternateApertureIntoBlocks(alternateKinematicSummaryHisto); 
...
% calculate averages
avgAperture = nanmean(alternateAperture(:,grpSess),2); % average each row

for i = 1:5 % calculate average first reach laser off, second reach laser off, etc. 
    avgOff(i,1) = nanmean(avgAperture(i:12:end));
end 

for i = 7:11   % laser on averages 
    avgOn(i-6,1) = nanmean(avgAperture(i:12:end));
end 

% collect individual rat data into matrix
for i_sess = grpSess
    for i = 1:5     % laser off data in rows 1-5, each column different session
        indiData(i,i_sess) = nanmean(alternateAperture(i:12:end,i_sess));
    end 
end

for i_sess = grpSess
    for i = 7:11    % laser on data in rows 6-10
        indiData(i-1,i_sess) = nanmean(alternateAperture(i:12:end,i_sess));
    end 
end

% get data set up to calculate std dev
aData = alternateAperture(:,grpSess);
offData = NaN(100,5);

rowSt = 1; % off trials
for row = 1:12:size(aData,1)
    col = 1;
    for i_row = row:row+4
        if i_row > size(aData,1)
            continue
        else    % column 1-5 = data from all sessions reach 1, reach 2, etc. in laser off blocks
            offData(rowSt:rowSt+size(aData,2)-1,col) = aData(i_row,:)';
            col = col+1;
        end
    end 
    rowSt = rowSt + size(aData,2);
end 

onData = NaN(100,5);
rowSt = 1; % on trials
for row = 7:12:size(aData,1)
    col = 1;
    for i_row = row:row+4
        if i_row > size(aData,1)
            continue
        else      % column 1-5 = data from all sessions reach 1, reach 2, etc. in laser on blocks
            onData(rowSt:rowSt+size(aData,2)-1,col) = aData(i_row,:)';
            col = col+1;
        end
    end 
    rowSt = rowSt + size(aData,2);
...
set(gca,'xtick',[1 5 6 10],'ytick',[5 15 25]);
