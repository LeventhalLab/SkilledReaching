function h_fig = plotOrientTrajAtDistOutcome(exptOutcomeSummary,i_grp,timeStamp)
...
for i_out = 1:8
    for i_pt = 1:size(data,2)
        avgData(:,i_pt,i_out) = nanmean(data(:,i_pt,i_out,:),4);        
    end
end 

for i_out = 1:8
    for i_sess = 1:22
        numDataPts = sum(~isnan(data(i_sess,timeStamp,i_out,:)),4);
        errbars(i_sess,i_out) = nanstd(data(i_sess,timeStamp,i_out,:),0,4)./sqrt(numDataPts);
    end
end 
...
% figure properties
box off
ylabel({'\theta (deg) at';'z_{digit2} coordinate'},'FontSize',10)
