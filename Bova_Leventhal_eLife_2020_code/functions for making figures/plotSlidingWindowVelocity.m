function plotSlidingWindowVelocity(indivSlidingWindow,session,i,yMin,yMax,numBins)

ratGrp = indivSlidingWindow(i).exptInfo.type;   % define colors for groups
...
numRats = size(indivSlidingWindow(i).velocity,3);

data = indivSlidingWindow(i).velocity(:,:,:);
data(data == 0) = NaN;
