...
function  [index_x,index_y,index_z,middle_x,middle_y,middle_z,ring_x,ring_y,ring_z,pinky_x,pinky_y,pinky_z]= analyzeManualTrialData(RatData)
   ...
   for i = 1:length(allPawData)
           ...
           [indexProx3]= create3Dpoints(indexProxLeft, indexProxCenter, indexProxLeft);
   ...
   distancePinkyDisttoPellet = [];
    ...
    for i = 1:length(allIndexDist3(:,1))
        for j = 1:5
            ...
            %cant do comparison of double arrays
            if size(currentPellet) == [1,3]
